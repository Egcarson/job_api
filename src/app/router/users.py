...
@user_router.get("/users", status_code=status.HTTP_200_OK, response_model=List[schemas.User])
async def get_all_users(role: Optional[schemas.UserRoles] = None, session: AsyncSession = Depends(get_session)):

    users = await user_service.get_all_users(session, role)
    return users

@user_router.get("/users/{user_uid}", status_code=status.HTTP_200_OK, response_model=schemas.UserDetails)
async def get_user(user_uid: UUID, session: AsyncSession = Depends(get_session), current_user=Depends(access_token_bearer)):
...
async def update_user(user_data: schemas.UserUpdate, user_uid: UUID, session: AsyncSession = Depends(get_session), token_details: dict =Depends(access_token_bearer)):
